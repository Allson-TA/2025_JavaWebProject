...
public class AuthCodeServlet extends HttpServlet {
	...
	private String generateAuthCode() {
		...
		String chars = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ";
		StringBuffer authcode = new StringBuffer();
		Random random = new Random();
		for(int i=0;i<4;i++) {
			int index = random.nextInt(chars.length());	//	隨機取得位置
	...
	*/
	...
	private BufferedImage getAuthCodeImage(String authcode) {
		...
		//	繪文字
		g.drawString(authcode, 18, 22);	//	22,22 --> 表示繪文字左上角的起點
		//	加上干擾線 (怕被其他輕易解讀)
